<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
     <%@ page import="com.feedback.FeedbackDBUtill" %>
     <%@ page import="com.feedback.FeedbackInsertController" %>
...
<link rel="stylesheet" href="./CSS/FeedbackDisplay.css">
...
<nav>
	...
	<main>
	<%
	List<Feedback> feedback = FeedbackDBUtill.getFeedback();
	request.setAttribute("feedback", feedback);
	%>
	<c:forEach var="data" items="${feedback}">
		<div class="feedbackCard">
		<div class="card">
		<div class="text">
		<h3></h3><br>
		<h3>${data.getName()}</h3>
		<h3>${data.getEmail()}</h3><br>
		<p>${data.getFeedback()}</p>
		<c:url value="FeedbackUpdate.jsp" var="feedUpdate">
			<c:param name="id" value="${data.getFeedbackid()}"></c:param>
			<c:param name="name" value="${data.getName()}"></c:param>
			<c:param name="email" value=">${data.getEmail()}"></c:param>
			<c:param name="contact" value=">${data.getContact()}"></c:param>
			<c:param name="Feedback" value="${data.getFeedback()}"></c:param>
		</c:url>
		<a href="${feedUpdate}">
			<input type="submit" value="Edit" class="feedbtn">
		</a>
		<c:url value="FeedbackDelete.jsp" var="feedDelete">
    	<c:param name="id" value="${data.getFeedbackid()}"></c:param>
   		</c:url>
    	<a href="${feedDelete}">
    		<input type="submit" value="Delete" class="feedbtn">
    	</a>
